---
name: kb-chunk-query
description: "Utility 层：安全查询 tz.kb_chunk 文档分块。用于按 document_id 或 chunk_id 读取 chunk 内容、摘要、页码、bbox、语言和 extra_meta。"
---
...
Utility 层：安全查询 tz.kb_chunk 文档分块。用于按 document_id 或 chunk_id 读取 chunk 内容、摘要、页码、bbox、语言和 extra_meta。
...
- `scripts/query_chunk.py` imports `none`
...
```bash
python scripts/query_chunk.py --document-id 623739c36e7c4ecb97397feeba3454bf --limit 3
```
