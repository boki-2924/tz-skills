---
name: health-check-renderer
description: "Utility 层：将知识库巡检 JSON 渲染为中文健康看板 HTML。用于展示文档、chunk、embedding、MinIO 对象和异常清单。"
---
...
Utility 层：将知识库巡检 JSON 渲染为中文健康看板 HTML。用于展示文档、chunk、embedding、MinIO 对象和异常清单。
...
- `scripts/render_health_dashboard.py` imports `make_run_dir, read_json, write_html, print_json`
...
```bash
python scripts/render_health_dashboard.py --input health.json
```
