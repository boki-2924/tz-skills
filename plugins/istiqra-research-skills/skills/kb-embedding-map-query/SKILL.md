---
name: kb-embedding-map-query
description: "Utility 层：安全查询 tz.kb_embedding_mapping 向量映射表。用于查看 document/chunk/qa embedding 模型、维度、Milvus collection、主键和索引状态。"
---
...
Utility 层：安全查询 tz.kb_embedding_mapping 向量映射表。用于查看 document/chunk/qa embedding 模型、维度、Milvus collection、主键和索引状态。
...
- `scripts/query_embedding.py` imports `none`
...
```bash
python scripts/query_embedding.py --document-id 623739c36e7c4ecb97397feeba3454bf --limit 5
```
