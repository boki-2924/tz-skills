---
name: doris-catalog-inspect
description: "Utility 层：安全查看 Doris/Iceberg catalog、database、table、column 元数据。用于发现数据目录和表结构，不执行写入。"
---
...
Utility 层：安全查看 Doris/Iceberg catalog、database、table、column 元数据。用于发现数据目录和表结构，不执行写入。
...
- `scripts/inspect.py` imports `none`
...
```bash
python scripts/inspect.py catalogs
```
